...
export type SearchInput<T> =
	| string
	| RegExp
	| ((data: string, rowData: T) => boolean);
export type SearchInputColumn<T> =
	| string
	| RegExp
	| ((data: string, rowData: T, column: number) => boolean);
...
type Layout = Partial<
	Record<LayoutKeys, LayoutElement | LayoutFeatures | null>
>;
...
export interface Api<T = any> {
	...
	iterator(
		type: "table",
		callback: InteratorTable,
		returns?: boolean,
	): Api<any>;
	iterator(
		type: "cell",
		callback: InteratorCell,
		returns?: boolean,
	): Api<any>;
	iterator(
		type: "column-rows",
...
export interface ApiCellsMethods<T>
	extends Omit<Api<T>, "data" | "render" | "select"> {
	/**
...
export interface ApiColumnMethods<T>
	extends Omit<Api<T>, "init" | "data" | "order" | "render" | "search"> {
	/**
...
export interface ApiStatic {
	 ...
	 */
	new (
		selector: string | Node | Node[] | JQuery | InternalSettings,
	): Api<any>;
...
export type ExtTypeSettingsDetect =
	| ((data: any, settings: InternalSettings) => boolean | string | null)
	| {
			/**
